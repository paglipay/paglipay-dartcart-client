import {
  ...
  const PRAdapter = createEntityAdapter(); // Entity is mapped to our Model. Create Entity Adapter provides REDUCERS
  ...
  } = PRAdapter.getSelectors(() => state.ProductReviews);
  
  export const getSearchString = createSelector(
    (state) => state.ProductReviews.searchString,
    (search) => {
      return search;
    }
  );
  ...
  export const getStatus = createSelector(
    (state) => state.ProductReviews.status,
    (status) => {
      return status;
    }
  );
